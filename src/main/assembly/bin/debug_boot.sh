...
#优先从环境变量目录中取配置文件
APP_CFG_FILE=$BPLAN_HOME/${PRG_NAME}/application.properties
APP_LOG_CFG_FILE=$BPLAN_HOME/${PRG_NAME}/logback-spring.xml
...
JAVA=java
JAVA_OPTS=" -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5000 -Djava.io.tmpdir=$WORK_DIR/tmp -Xms256m -Xmx512m -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:-CMSConcurrentMTEnabled -XX:CMSInitiatingOccupancyFraction=70 -XX:+CMSParallelRemarkEnabled -Dwork.dir=${WORK_DIR}"
CLASS_PATH=" -classpath "$(echo ${WORK_DIR}lib/*.jar|sed 's/ /:/g')

CLASS=com.liuz.bplan.BplanApplication
#CONFIG_PATH=" --spring.config.location=$APP_CFG_FILE --logging.config=$APP_LOG_CFG_FILE"
CONFIG_PATH=" --logging.config=$APP_LOG_CFG_FILE"
